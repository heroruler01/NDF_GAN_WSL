...
class MeshRenderer():
    ...
    def _initialize_opengl(self):
        ...
        self.shader = Shader()
        self.shader.initShader(open('/home/sebastian/ndf_gan/Shape_GAN/rendering/vertex.glsl').read(), open('/home/sebastian/ndf_gan/Shape_GAN/rendering/fragment.glsl').read())

        self.shadow_framebuffer = glGenFramebuffers(1)
        self.shadow_texture = create_shadow_texture()

        self.depth_shader = Shader()
        self.depth_shader.initShader(open('rendering/depth_vertex.glsl').read(),
                                     open('rendering/depth_fragment.glsl').read())
    ...
    def get_image(self, crop=False, output_size=None, greyscale=False, flip_red_blue=False):
        ...
        if greyscale:
            ...
            array = np.transpose(pygame.surfarray.array3d(image)[:, :, (2, 1, 0) if flip_red_blue else slice(None)],
                                 (1, 0, 2))
