...
from Shape_GAN.util import device, ensure_directory
from Shape_GAN.rendering.math import get_camera_transform
...
def get_normals(sdf_net, points, latent_code):
    ...
    for i in range(batch_count):
        result[BATCH_SIZE * i:BATCH_SIZE * (i + 1), :] = sdf_net.get_normals(latent_code,
                                                                             points[BATCH_SIZE * i:BATCH_SIZE * (i + 1),
                                                                             :])
...
def render_image(sdf_net, latent_code, resolution=800, threshold=0.0005, sdf_offset=0, iterations=1000, ssaa=2,
                 radius=1.0, crop=False, color=(0.8, 0.1, 0.1), vertical_cutoff=None):
    ...
    points[ground_points, :] -= ray_directions[ground_points, :] * ((points[ground_points, 1] - ground_plane) /
