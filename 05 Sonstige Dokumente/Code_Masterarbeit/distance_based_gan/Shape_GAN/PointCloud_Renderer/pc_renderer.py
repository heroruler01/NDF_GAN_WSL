...
def generate_point_cloud(num_steps=10, num_points=900000, filter_val=0.009):#0.009, good is 0.02 in combination with 0.06 below
    start = time.time()
    ...
    unit_sphere_points = np.random.uniform(-1.1, 1.1, size=(sample_num * 2, 3)).astype(np.float32)
    unit_sphere_points = unit_sphere_points[np.linalg.norm(unit_sphere_points, axis=1) < 1.1]
    ...
    while len(samples_cpu) < num_points:
        ...
        for j in range(num_steps):
            print('refinement', j)
            df_pred = torch.clamp(generator(samples, z), max=threshold).squeeze(0)
            df_pred = torch.abs(df_pred)
        ...
        samples = samples[df_pred < 0.05]#.unsqueeze(0)#0.03
