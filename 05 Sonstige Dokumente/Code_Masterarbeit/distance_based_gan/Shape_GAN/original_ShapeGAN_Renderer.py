...
import GAN.GAN_NDF_combine as NDF
...
class Renderer():
    def __init__(self, act_cam_or, act_cam_pos):
        self.act_cam_or = act_cam_or
        self.act_cam_pos = act_cam_pos
        self.get_args()
    ...
    def to_transf_matrix(self):
        ...
        inp_trans = np.reshape(self.act_cam_pos, (3,))
    ...
    def define_screen_points(self):
        ...
        screen_and_cam_hom = self.append_one(screen_and_cam) # [camx, camy, camz, 1]
                                                             # [pointx, y, z, 1] ...
        # 4 X SIZE^2
        screen_and_cam_hom_T = np.transpose(screen_and_cam_hom, (1, 0)) # wahrscheinlich nur damit die matrixmulti mit transf. matrix besser geht
    ...
    def get_lgth_rays(self):
        ...
        :return:
        """
        src_batch = np.repeat([self.args.light_position], self.args.size * self.args.size, axis=0) # hier repeat (self.cam_trans) liefer gleich Ergebnisse wie nur shade
    ...
    def run(self, model, device):
        ...
        """
        #print(self.args)
        #NDF.loadNDF(model, inputs, sd)

        depth = np.zeros((self.args.size * self.args.size, 1))
        cam_batch = np.repeat(self.cam_trans, self.args.size * self.args.size, axis=0)

        points = cam_batch.copy() # warum werden hier nicht die screen points eingesetzte??

        iter = 1
        ray = self.unit_rays.copy()
        indices_cont_all = list(range(self.args.size * self.args.size))
        # latent_vector_img = torch.randn(1, 3479, self.args.size * self.args.size, device=device)
        latent_vector_img = torch.randn(1, 128, device=device)
        #latent_vector_img = torch.randn(1, 128, self.args.size * self.args.size, device=device) # das ist normal der richtige
        ...
        while len(indices_cont_all) > 0:
            ...
            print('Iter:', iter)
            dists_points = np.abs(NDF.predictRotNDF(model, points, latent_vector_img, device))
            dists_points = np.reshape(dists_points, (self.args.size * self.args.size, 1)) # einfach ein Punkt pro zeile! Für alle Pixel
        ...
        dists, gradients = NDF.predictRotGradientNDF(model, points, latent_vector_img, device)
    ...
    def save(self, image, name, size, normalize):
        ...
        cv2.imwrite(os.path.join(self.args.folder, name), np.uint8(255 * stacked_img))
        return stacked_img
    ...
    def save_images(self):
        ...
        stacked_img = self.save(shade, 'shade_GAN.jpg', 1, True)
        return stacked_img
...
def pred_render(model, act_cam_or, act_cam_pos, device):
    start_time = time.time()
    renderer = Renderer(act_cam_or, act_cam_pos)
    renderer.run(model, device)
    pred_image = renderer.save_images()
