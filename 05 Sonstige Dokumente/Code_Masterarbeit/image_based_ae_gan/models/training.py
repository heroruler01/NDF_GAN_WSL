...
class Trainer(object):
    ...
    def compute_loss(self,batch):
        ...
        p = batch.get('grid_coords').to(device) #grid coords sind die gesampelten Punkte mit Sigma! also nicht direkt auf Oberfläche
        df_gt = batch.get('df').to(device) #(Batch,num_points) #(4, 50000)
        inputs = batch.get('inputs').to(device)#tatsächlichen Punkte der sparse point cloud von welcher NDF gelernt werden soll!
        # gibt für die voxel in 256/256/256 BB an ob sie belegt sind (1) oder nicht (0)


        df_pred = self.model(p,inputs) #(Batch,num_points) hier werden Distanzen der Punke der gesampelten Punkte mit sigma zu dem NDF das auf sparse point cloud encoded ist ausgeg.!
        # durch Vergleich der Distanzend er Punkte von NDF auf sparse PC zu GT NDF lernt dann NDF mit dem encoding die dekodierte Distanz richtig darzustellen
