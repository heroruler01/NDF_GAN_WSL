...
class NDF(nn.Module):
    ...
    # Input is (4,497,1,7,50000)
    def decoder(self, p, f_0, f_1, f_2, f_3, f_4, f_5, f_6):

        p_features = p.transpose(1, -1) #[4,3,50000]
        p = p.unsqueeze(1).unsqueeze(1) # p hat shape [4,1,1,50000,3]
        ...
        mini = torch.min(f_1)
        maxi = torch.max(f_1)
        features = torch.cat((features, p_features), dim=1)  # (B, featue_size, samples_num) # [4,3482,50000]
