...
def loadNDF(model):
    ...
    '''
'''
def loadNDF_origin(index, pointcloud_samples, exp_name, data_dir, split_file, sample_distribution, sample_sigmas, res,  mode = 'test'):
...
def predictRotNDF(model, points, latent_vector_img, device): #warum machen wir das hier so um dann am Ende wieder mit Funktion unten zurück zu gehen?

    points = rot_YZ(points)
    points = to_grid(points)
    points = torch.from_numpy(points).unsqueeze(0).float().to(device)
    return torch.clamp(model(points, latent_vector_img), max=0.1).squeeze(0).cpu().numpy()


def predictRotGradientNDF(model, points, latent_vector_img, device):
    points = rot_YZ(points)
    points = to_grid(points)
    points = torch.from_numpy(points).unsqueeze(0).float().to(device)
    points.requires_grad = True

    df_pred = torch.clamp(model(points, latent_vector_img), max=0.1)
