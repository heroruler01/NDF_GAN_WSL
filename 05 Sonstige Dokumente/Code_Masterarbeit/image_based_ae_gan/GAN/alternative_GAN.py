...
class SDFGenerator(torch.nn.Module):
    def __init__(self, latent_channels=128, hidden_channels=256, num_layers=8, norm=True,
                 dropout=0.0):
    ...
    def forward(self, p, latent_vector):
        ...
        pos = p.unsqueeze(0) if p.dim() == 2 else p

        assert pos.dim() == 3
        assert pos.size(-1) == 3

        z = latent_vector.unsqueeze(0) if latent_vector.dim() == 1 else latent_vector
        assert z.dim() == 2
