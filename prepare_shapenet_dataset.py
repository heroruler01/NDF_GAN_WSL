...
# 输入模型路径 (请确认你的 obj 文件确实在这个目录下)
DIRECTORY_MODELS = PROJECT_ROOT + '/data/shapenet/00_good'

MODEL_EXTENSION = '.obj' 

# 输出路径 (全部改为基于 PROJECT_ROOT)
DIRECTORY_VOXELS = PROJECT_ROOT + '/data/shapenet/{:s}/voxels_{{:d}}/'.format(DATASET_NAME)
DIRECTORY_UNIFORM = PROJECT_ROOT + '/data/shapenet/{:s}/uniform/'.format(DATASET_NAME)
DIRECTORY_SURFACE = PROJECT_ROOT + '/data/shapenet/{:s}/surface/'.format(DATASET_NAME)
DIRECTORY_SDF_CLOUD = PROJECT_ROOT + '/data/shapenet/{:s}/cloud/'.format(DATASET_NAME)
DIRECTORY_BAD_MESHES = PROJECT_ROOT + '/data/shapenet/{:s}/bad_meshes/'.format(DATASET_NAME)
# ================= 路径配置修改结束 =================
...
POINT_CLOUD_SAMPLE_SIZE = 64 ** 3  # For uniform and surface points (CREATE_UNIFORM_AND_SURFACE) (eigentlich 64 ** 3)
...
def get_hash(filename):
    return filename.split('/')[-1] # -2 normalerweise!!
...
def get_uniform_and_surface_points(surface_point_cloud, number_of_points=200000):
    unit_sphere_points = np.random.uniform(-1.2, 1.2, size=(number_of_points * 2, 3)).astype(np.float32)
    unit_sphere_points = unit_sphere_points[np.linalg.norm(unit_sphere_points, axis=1) < 1.2]
...
def process_model_file(filename):
    try:
        ...
        if create_uniform_and_surface or create_sdf_clouds:
            if not os.path.exists(get_uniform_filename(filename)):
                ...
                try:
                    if create_uniform_and_surface:
                        ...
                        #alternative way to compute uniform points!
                        unit_sphere_points = np.random.uniform(-1.2, 1.2, size=(POINT_CLOUD_SAMPLE_SIZE * 2, 3)).astype(np.float32)
                        unit_sphere_points = unit_sphere_points[np.linalg.norm(unit_sphere_points, axis=1) < 1.2]
                        uniform_points = unit_sphere_points[:POINT_CLOUD_SAMPLE_SIZE, :]

                        # ----------------------------------------
                        # incorporate near surface sampling from Chibane
                        # ----------------------------------------
                        sample_num = 20000
                        sigma = 0.003
                        points = mesh_unit_sphere.sample(sample_num)
                        ...
                        complete_points = np.concatenate((uniform_points, boundary_points))


                        uniform_sdf = igl.signed_distance(uniform_points, mesh_unit_sphere.vertices, mesh_unit_sphere.faces)[0]
                       ...
                       # ================= 修复开始：补全表面点采样逻辑 =================
                       # 1. 在球面上采样点
                        surface_samples, _ = trimesh.sample.sample_surface(mesh_unit_sphere, POINT_CLOUD_SAMPLE_SIZE)
 
                       # 2. 添加微小的高斯噪声
                        near_surface_points = surface_samples + np.random.normal(scale=0.0025, size=surface_samples.shape).astype(np.float32)

                       # 3. 计算这些点的 SDF 值 (符号距离)
                        near_surface_sdf = igl.signed_distance(near_surface_points, mesh_unit_sphere.vertices, mesh_unit_sphere.faces)[0]
...
def process_model_files():
    ...
    print("正在使用【直接调试模式 (无 Pool)】运行...")
    print("------------------------------------------------")

    # ❌ [删除] 原来的 pool = Pool(...) 全部不要了
    ...
    for i, filename in enumerate(files):
        # 这一行会在处理每个文件前打印名字
        # 如果卡住，屏幕上显示的最后一行就是那个坏掉的文件
        # tqdm.write(f"正在处理: {get_hash(filename)}") 
        
        # 直接调用函数，不再通过 pool.apply_async
